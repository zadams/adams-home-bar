#!/usr/bin/env node
/**
 * Generate one prompt .txt per cocktail from the illustration manifest.
 *
...
const outputDir =
  process.argv[3] ?? path.join(root, 'docs/illustration-system/generated-prompts')
...
console.log(`Generated ${manifest.cocktails.length} prompt files in ${path.relative(root, outputDir)}`)
console.log('See docs/illustration-system/ILLUSTRATION_BIBLE.md before generating art.')
